...
struct Parking {
    ...
    private var register: (vehicle: Int, fee: Int) = (vehicle: 0, fee: 0)
    ...
    func calculateFee(type: VehicleType,hasDiscountCard:Bool, parkedTime: Int) -> Int{
       let timeInitial = 120
        var buy = 0
        let excessTime = (Int(ceil(Double(parkedTime - timeInitial) / 15)) * 5)
        ...
        if parkedTime <= timeInitial {
            ...
            buy = (Int(floor(Double(excessTime) + Double(type.rate) ) * 0.85))
            return buy
        }
        buy = excessTime + type.rate
        return buy
   }
